...
if ($_REQUEST['work_start'] && check_bitrix_sessid()) {
    ...
    $rsEl = CIBlockElement::GetList(array("ID" => "ASC"), array("IBLOCK_ID" => $BID, ">ID" => $_REQUEST["lastid"]), false, array("nTopCount" => $limit), ['ID', 'CODE','PROPERTY_CML2_ARTICLE']);

    while ($recount = $rsEl->Fetch()) {
        $recountId = $recount['ID'];
        if ($recount['PROPERTY_CML2_ARTICLE_VALUE']){
            $el->update($recountId, ['CODE'=>$recount['PROPERTY_CML2_ARTICLE_VALUE']]);
        }
...
$APPLICATION->SetTitle("Установка артикула в символьный код");
