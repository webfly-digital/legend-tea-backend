...
if ($_REQUEST['work_start'] && check_bitrix_sessid()) {
    ...
    $rsEl = CIBlockElement::GetList(array("ID" => "ASC"), array("IBLOCK_ID" => $BID, ">ID" => $_REQUEST["lastid"]), false, array("nTopCount" => $limit), ['ID', 'CODE','PROPERTY_CODE']);

    while ($recount = $rsEl->Fetch()) {
        $recountId = $recount['ID'];
        if ($recount['CODE']){
            CIBlockElement::SetPropertyValuesEx($recountId, $BID, array('CODE' => $recount['CODE']));
        }
...
$APPLICATION->SetTitle("Установка символьного кода в свойство");
