...
if ($_REQUEST['work_start'] && check_bitrix_sessid()) {
    // if (!$_REQUEST["lastid"])
    // $_REQUEST["lastid"] = 2330;
    $rsEl = CIBlockElement::GetList(array("ID" => "ASC"), array("IBLOCK_ID" => $BID, ">ID" => $_REQUEST["lastid"]), false, array("nTopCount" => $limit), ['ID', 'CODE','PROPERTY_CODE', 'DETAIL_PAGE_URL']);

    while ($recount = $rsEl->GetNext()) {
        $recountId = $recount['ID'];

        $explode = explode('/', trim($recount['DETAIL_PAGE_URL'], '/'));
        if ($explode){
            unset($explode[count($explode)-1]);
            $oldUrl = '/'.implode('/', $explode).'/'.$recount['PROPERTY_CODE_VALUE'].'/';
            $file = $_SERVER['DOCUMENT_ROOT'] . '/auto/redirect.json';
            $jsonData = json_decode(file_get_contents($file), true);
            $handle = fopen($file, 'w'); // Открываем файл для записи и стираем его содержимое
            fwrite($handle, json_encode(array_merge($jsonData, [$oldUrl=>'/product/'.$recount['CODE'].'/']))); // Сохраняем в файл
            fclose($handle); // Закрываем файл
        }
        //запоминаем последний ID
        $lastID = intval($recountId);
    }
