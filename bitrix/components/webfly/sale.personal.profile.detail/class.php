...
class WFPersonalProfileDetail extends CBitrixComponent
{
    ...
    public function onPrepareComponentParams($params)
    {
        ...
        if (isset($params['PATH_TO_LIST']) && $this->idProfile <= 0) {
            $this->newProfile = true;
            $params['PATH_TO_LIST'] = trim($params['PATH_TO_LIST']);
        } elseif ($this->idProfile) {
            $params['PATH_TO_LIST'] = '/personal/profiles/';
        } else {
            $this->newProfile = true;
            //return false;
        }
    ...
    public function executeComponent()
    {
        ...
        if ($this->arParams["SET_TITLE"] === 'Y') {
            if ($this->newProfile)
                $APPLICATION->SetTitle('Новая компания');
            else
                $APPLICATION->SetTitle(Loc::getMessage("SPPD_TITLE") . $this->idProfile);
        }

        if (($this->idProfile <= 0 || $request->get('reset')) && !$this->newProfile) {
        ...
        if ($this->errorCollection->isEmpty()) {
            ...
            if ($this->newProfile) {
                $pType = $request->getQuery("profile-type") ?: B2B_UR_PERSON_TYPE_ID;
                $userOrderProperties = ['ID' => 'NEW', 'NAME' => 'Новая компания', 'USER_ID' => (int)($USER->GetID()), 'PERSON_TYPE_ID' => $pType];
            } else {
                ...
                $userOrderProperties = $userProperties->fetch($htmlConvector);
            }
            if ($userOrderProperties) {
    ...
    protected
    function getIblockData()
    {
        global $USER;
        if ($this->idProfile > 0) {
            $arElement = \CIBlockElement::getList([],
                ['IBLOCK_ID' => B2B_COMPANIES, '=PROPERTY_PROFILE_ID' => $this->idProfile, '=PROPERTY_USER_ID' => (int)($USER->GetID())], false, false,
                ['ID', 'NAME', 'PREVIEW_PICTURE'])->fetch();
        }


        if ($arElement['ID']) $this->element = $arElement;

        /**
         * Get IB props
         */
        $rsIBLockPropertyList = \CIBlockProperty::GetList(array("sort" => "asc", "name" => "asc"), array("ACTIVE" => "Y", "IBLOCK_ID" => B2B_COMPANIES));
        while ($arProperty = $rsIBLockPropertyList->GetNext()) {
            ...
            $this->iblockProperties[$arProperty["CODE"]] = $arProperty;
        }
    }
     ...
     */
    protected
    function updateProfileProperties($request, $userOrderProperties)
    {
     ...
     */
    protected
    function fillResultArray($property)
    {
     ...
     */
    protected
    function executeUpdatingProperties($request, $fieldValues)
    {

        if ($this->errorCollection->isEmpty()) {
            $saleProps = new \CSaleOrderUserProps;
            if ($this->newProfile) {
                if (!$this->idProfile = $saleProps->add(array(
                    "NAME" => trim($request->get("NAME")),
                    "PERSON_TYPE_ID" => trim($request->get("PERSON_TYPE_ID")),
                ))) {
        ...
        if ($this->newProfile) {
            foreach ($fieldValues as $value) {
                ...
                $value['USER_PROPS_ID'] = $this->idProfile;
                $value['VALUE'] = is_array($value['VALUE']) ? current($value['VALUE']) : $value['VALUE'];
            ...
            while ($propertyValues = $userPropertiesList->Fetch()) {
                ...
                if (isset($fieldValues[$propertyValues["ORDER_PROPS_ID"]]['VALUE'])) {
                    $res = Sale\Internals\UserPropsValueTable::update(
                        $propertyValues["ID"],
     ...
     */
    protected
    function deleteFromPropertyTypeFile($idFileDeletingList, $baseArray)
    {
		if($baseArray){
