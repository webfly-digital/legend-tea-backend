...
<div class="maxwidth-theme bottom-info-wrapper">
                                    ...
                                    <div class="item_info">
                                        <div class="item-title">
                                            <a href="<?= $arSetItem["DETAIL_PAGE_URL"] ?>"
                                               class="dark_link font_xs"><span><?= $arSetItem["NAME"] ?></span></a>
                                        </div>
            ...
            <div class="ordered-block js-store-scroll tabs-block">
                            ...
                            <? foreach ($arTabOrder as $value): ?>
                                ...
                                <? if ($value == "desc"): ?>
                                    <? if ($bShowDetailTextTab || ($arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB" && $bShowPropsTab)): ?>
                                        <li class="bordered rounded3 <?= (!($iTab++) ? 'active' : '') ?>"><a
                                                    href="#desc"
                                ...
                                <? if ($value == "buy"): ?>
                                    <? if ($bShowHowBuyTab): ?>
                                        <li class="bordered rounded3 <?= (!($iTab++) ? 'active' : '') ?>"><a
                                                    href="#buy"
                                                    data-toggle="tab"><?= $arParams["TITLE_HOW_BUY"]; ?></a>
                                        </li>
                                    <? endif; ?>
                                <? endif; ?>
                                <? //show payment block
                                ...
                                <? if ($value == "payment"): ?>
                                    <? if ($bShowPaymentTab): ?>
                                        <li class="bordered rounded3 <?= (!($iTab++) ? 'active' : '') ?>"><a
                                                    href="#payment"
                                                    data-toggle="tab"><?= $arParams["TITLE_PAYMENT"]; ?></a></li>
                                    <? endif; ?>
                                <? endif; ?>
                                <? //show delivery block
                                ?>
                                <? if ($value == "delivery"): ?>
                                    <? if ($bShowDeliveryTab): ?>
                                        <li class="bordered rounded3 <?= (!($iTab++) ? 'active' : '') ?>"><a
                                                    href="#delivery"
                                                    data-toggle="tab"><?= $arParams["TITLE_DELIVERY"]; ?></a></li>
                                    <? endif; ?>
                                <? endif; ?>
                                <? //show stores block
                                ?>
                                <? if ($value == "stores"): ?>
                                    <? if ($bShowStoresTab): ?>
                                        <li class="bordered rounded3 <?= (!($iTab++) ? 'active' : '') ?>"><a
                                                    href="#stores"
                                                    data-toggle="tab"><?= $arParams["TAB_STOCK_NAME"]; ?></a></li>
                                    <? endif; ?>
                                <? endif; ?>
                                ...
                                <? if ($value == "custom_tab"): ?>
                                    <? if ($bShowCustomTab): ?>
                                        <li class="bordered rounded3 <?= (!($iTab++) ? 'active' : '') ?>"><a
                                                    href="#custom_tab"
                                                    data-toggle="tab"><?= $arParams["TAB_DOPS_NAME"]; ?></a></li>
                                    <? endif; ?>
                                <? endif; ?>
                                <? //show reviews block
                                ?>
                                <? if ($value == "reviews"): ?>
                                    <? if ($bShowReviewsTab): ?>
                                        <li class="bordered rounded3 <?= (!($iTab++) ? 'active' : '') ?>">
                                            <a href="#reviews"
                                               data-toggle="tab"><?= $arParams["TAB_REVIEW_NAME"]; ?><? $APPLICATION->ShowViewContent('PRODUCT_REVIWS_COUNT_INFO') ?></a>
                                        </li>
                                    <? endif; ?>
                                <? endif; ?>
                                <? //show video block
                ...
                <div class="tab-content<?= ($i <= 1 ? ' not_tabs' : '') ?>">
                    ...
                    <? foreach ($arTabOrder as $value): ?>
                        ...
                        <? if ($value == "payment"): ?>
                            <? if ($bShowPaymentTab): ?>
                                <div class="tab-pane <?= $value; ?> <?= (!($iTab++) ? 'active' : '') ?>"
                                     id="payment">
                                    <? if ($i == 1): ?>
                                        <div class="ordered-block__title option-font-bold font_lg">
                        ...
                        <? if ($value == "reviews"): ?>
                            <? if ($bShowReviewsTab): ?>
                                <div class="tab-pane <?= $value; ?> <?= $arParams['REVIEWS_VIEW'] ?> <?= (!($iTab++) ? 'active' : '') ?>"
                                     id="reviews">
                                    ...
                                    <div id="reviews_content"
                                         class="<?= $arParams['REVIEWS_VIEW'] == 'EXTENDED' ? '' : 'bordered rounded3' ?>">
            ...
            <? if ($arParams['REVIEWS_VIEW'] == 'EXTENDED'): ?>
                <div class="reviews-title__wrapper">
                    ...
                    <div class="right_reviews_info">
                        ...
                        <?
                        global $USER;
                        $addReview = \Webfly\Helper\Functions::checkPurchasedProduct($arResult["ID"]);
                        if ($USER->IsAuthorized() && $addReview):?>
                            <div class="show-comment btn btn-xs btn-default">
                                <?= GetMessage('ADD_REVIEW') ?>
                            </div
                        <? endif; ?>
                    ...
                    <? if ($USER->IsAuthorized() && !$addReview): ?>
                        <div>
                            Оставить отзыв на товар можно при наличии как минимум одного заказа.
                        </div>
                    <? endif; ?>
                    <? if (!$USER->IsAuthorized()):
                        ...
                        ?>
                        <div>
                            Для отправки отзыва вам нужно <a data-event="jqm" data-param-type="auth"
                                                             data-param-backurl="<?= htmlspecialcharsbx($url) ?>"
                                                             data-name="auth" href="/personal/">авторизоваться</a>.
                            Оставить отзыв можно при наличии как минимум одного заказа.
                        </div>
                    <? endif; ?>
                </div>
            <? else: ?>
    ...
    ?>
        <div class="ordered-block <?= $code ?>">
                ...
                <ul class="nav nav-tabs">
                    <? if ($bShowAssociatedTab): ?>
                        <li class="active"><a href="#assoc" data-toggle="tab"
                                              class="linked"><?= $arParams["DETAIL_ASSOCIATED_TITLE"]; ?></a>
                        </li>
                    <? endif; ?>
                    <? if ($bShowExpandablesTab): ?>
                        <li class="<?= $bShowAssociatedTab ? '' : 'active' ?>"><a href="#expandables"
                                                                                  data-toggle="tab"
                                                                                  class="linked"><?= $arParams["DETAIL_EXPANDABLES_TITLE"]; ?></a>
                        </li>
                    <? endif; ?>
                </ul>
            </div>
                ...
                <div class="tab-pane active cur" id="assoc">
                    ...
                    <div class="cur">
                        ...
                        <div class="assoc-block js-load-block loader_circle" data-block="assoc"
                             data-file="<?= $APPLICATION->GetCurPage() ?>">
                            <div class="stub"></div>
            ...
            <div class="left_block sticky-sidebar-custom product-side">
                ...
                <? if ($arParams['USE_BIG_DATA'] == 'Y'): ?>
                    ...
                    \Webfly\Helper\Functions::changeFilterRoz($arrFilterBigData);
                    ...
                    <div class="side-block side-block--feedback rounded2 bordered box-shadow colored_theme_hover_bg-block">
                        ...
                        <div class="side-block__bottom side-block__bottom--last">
                            <span class="btn btn-lg btn-transparent round-ignore btn-wide font_upper animate-load colored_theme_hover_bg-el has-ripple"
                                  ...
                                  data-name="ask"><?= ($arParams["ASK_TAB"] ? $arParams["ASK_TAB"] : GetMessage("ASK_TAB")) ?></span>
                        </div>
                    </div>
                <? endif; ?>
            </div>
        ...
        <?
        $des = new \Bitrix\Main\Page\FrameStatic('des');
        $des->startDynamicArea(); ?>
        <script>
