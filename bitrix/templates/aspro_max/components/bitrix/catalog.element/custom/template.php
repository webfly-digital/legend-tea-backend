...
// Webfly new
CustomOffers::GetSKUPropsArray($arResult['SKU_PROPS'], $arResult["SKU_IBLOCK_ID"], $arResult);
            ...
            <div class="product-detail-gallery swipeignore">
                <div class="product-detail-gallery__container<?= ($bVertical ? ' product-detail-gallery__container--vertical' : ''); ?>">
                    ...
                    <link href="<?= ($arFirstPhoto["BIG"]["src"] ? $arFirstPhoto["BIG"]["src"] : $arFirstPhoto["SRC"]); ?>"
                          itemprop="image"/>
                    <div class="product-detail-gallery__slider<? if (!$bMagnifier): ?> owl-carousel owl-theme big owl-bg-nav short-nav<? else: ?> hidden-xs<? endif; ?> <?= $arParams['PICTURE_RATIO']; ?>"
                            ...
                            <div id="photo-sku"
                                ...
                                <? if ($arFirstPhoto["BIG"]["src"]): ?>
                                    <a href="<?= ($viewImgType == "POPUP" ? $arFirstPhoto["BIG"]["src"] : "javascript:void(0)"); ?>" <?= ($bIsOneImage ? '' : 'data-fancybox="gallery"') ?>
                                       class="product-detail-gallery__link <?= ($viewImgType == "POPUP" ? "popup_link fancy" : "line_link"); ?>"
                                       title="<?= $title; ?>">
                                    ...
                                    <div id="photo-<?= $i ?>"
                                        ...
                                        <? if (!$isEmpty) {
                                            ...
                                            <a href="<?= ($viewImgType == "POPUP" ? $arImage["BIG"]["src"] : "javascript:void(0)"); ?>" <?= ($bIsOneImage ? '' : 'data-fancybox="gallery"') ?>
                                               class="product-detail-gallery__link <?= ($viewImgType == "POPUP" ? "popup_link fancy" : "line_link"); ?> "
                                               title="<?= $title; ?>">
                                                <img class="lazy product-detail-gallery__picture <?= ($viewImgType == "MAGNIFIER" ? "zoom_picture" : ""); ?>"
                    ...
                    <? if ($bMagnifier): ?>
                        <div class="product-detail-gallery__slider owl-carousel owl-theme big visible-xs <?= $arParams['PICTURE_RATIO']; ?>"
                    ...
                    <div class="product-detail-gallery__thmb-container text-center">
                        <div class="product-detail-gallery__thmb-inner<?= ($bVertical ? ' vertical' : ''); ?>">
                            ...
                            <? if (!$showCustomOffer || empty($arResult['OFFERS_PROP'])): ?>
                                <? if ($countPhoto > 1): ?>
                                    <div class="product-detail-gallery__slider owl-carousel owl-theme thmb<?= ($bVertical ? ' product-detail-gallery__slider--vertical' : ''); ?><?= ($countPhoto > 3 ? ' m-photo' : ''); ?>"
                                ...
                                <? if ($arResult['PROPERTIES']['POPUP_VIDEO']['VALUE']): ?>
                                    <div class="video-block popup_video <?= ($countPhoto > 3 ? 'fromtop' : ''); ?> sm">
                                        <a class="various video_link image dark_link"
                                           href="<?= $arResult['PROPERTIES']['POPUP_VIDEO']['VALUE']; ?>"
                                           title="<?= Loc::getMessage("VIDEO") ?>"><span
                                                    class="play text-upper font_xs"><?= Loc::getMessage("VIDEO") ?></span></a>
                                    </div>
                                <? endif; ?>
                            <? else: ?>
                                <div class="product-detail-gallery__slider owl-carousel owl-theme thmb<?= ($bVertical ? ' product-detail-gallery__slider--vertical' : ''); ?>"
                                     data-size="<?= $countPhoto; ?>"
                                     data-plugin-options='{"items": "4", "nav": true, "loop": false, "clickTo": ".product-detail-gallery__slider.big", "dots": false, "autoWidth": true, "margin": 10<? //if($bVertical):?>, "mouseDrag": false, "pullDrag": false<? //endif;?><? if ($bMagnifier): ?>, "magnifier": true<? endif; ?>}'
                                     style="max-width:<?= ceil((($countPhoto <= 4 ? $countPhoto : 4) * 70) - 10) ?>px;">
                                </div>
                            ...
                            <div class="product-info-headnote__inner">
                                ...
                                <? if ($arResult["BRAND_ITEM"]) { ?>
                                    <div class="product-info-headnote__brand">
                                        <div class="brand">
                                            <meta itemprop="brand"
                                                  content="<?= $arResult["BRAND_ITEM"]["NAME"] ?>"/>
                                    ...
                                    <div class="sku_props">
                                        <div id="<?= $itemIds['TREE_ID'] ?>">
                                            <? foreach ($arResult['SKU_PROPS'] as $skuProperty):
                                                ...
                                                <? if ($skuProperty['CODE'] == 'POMOL'):
                                                $activePomol = array_filter($skuProperty['VALUES'], function ($var) {
                                                    return $var['CLASS'] == 'active';
                                                }); ?>
                                                <? if (empty($activePomol)) {
                                                $activePomol = array_filter($skuProperty['VALUES'], function ($var) {
                                                        return $var['CLASS'] == 'active missing';
                                                });
                                            }
                                                ?>
                                                <div data-id="<?= $propertyId ?>" data-display_type="DROPDOWN-DIV"
                                                     id="<?= $itemIds['PROP_ID'] ?>_<?= $skuProperty['ID'] ?>_cont"
                                                     class="dropdown-select pomol_select" <?= $skuProperty['STYLE'] ?>>
                                                    <span>Выберите помол</span>
                                                    <div class="dropdown-select__title font_xs darken">
                                                        <span><?= current($activePomol)['NAME'] ? htmlspecialcharsbx(current($activePomol)['NAME']) : '' ?></span>
                                                    </div>
                                                    <div class="dropdown-select__list dropdown-menu-wrapper"
                                                         role="menu">
                                                        <div class="dropdown-menu-inner rounded3"
                                                             id="<?= $itemIds['PROP_ID'] ?>_<?= $skuProperty['ID'] ?>_list">
                                                            <? foreach ($skuProperty['VALUES'] as &$value): // dropdown-select__list-link--current
                                                                $value['NAME'] = htmlspecialcharsbx($value['NAME']); ?>
                                                                <div class="dropdown-select__list-item font_xs" <?= $value['STYLE'] ?>>
                                                                    <dropdown-div data-showtype="DROPDOWN-DIV"
                                                                                  title="<?= $value['NAME'] ?>"
                                                                                  data-treevalue="<?= $propertyId ?>_<?= $value['ID'] ?>"
                                                                                  data-onevalue="<?= $value['ID'] ?>"
                                                                                  class="dropdown-select__list-link <?= $value['CLASS'] ?>"><?= $value['NAME'] ?></dropdown-div>
                                                                </div>
                                                            <? endforeach ?>
                                                        </div>
                                                    </div>
                                            ...
                                            <? else: ?>
                                                <div data-id="<?= $propertyId ?>" data-display_type="LI"
                                                     id="<?= $itemIds['PROP_ID'] ?>_<?= $skuProperty['ID'] ?>_cont" <?= $skuProperty['STYLE'] ?>>
                                                    <ul class="list_values_wrapper"
                                                        id="<?= $itemIds['PROP_ID'] ?>_<?= $skuProperty['ID'] ?>_list">
                                                        <? foreach ($skuProperty['VALUES'] as &$value):
                                                            $value['NAME'] = htmlspecialcharsbx($value['NAME']); ?>
                                                            <li class="<?= $value['CLASS'] ?>" data-showtype="LI"
                                                                title="<?= $value['NAME'] ?>"
                                                                data-treevalue="<?= $propertyId ?>_<?= $value['ID'] ?>"
                                                                data-onevalue="<?= $value['ID'] ?>" <?= $value['STYLE'] ?>>
                                                                <span class="cnt"><?= $value['NAME'] ?></span>
                                                                <? if ($value['PRICE'] > 0): ?>
                                                                    <span class="sku-price_additional"><?= $value['PRICE'] ?></span>
                                                                <? endif ?>
                                                            </li>
                                                        <? endforeach ?>
                                                    </ul>
                        ...
                        <div class="info_item">
                            <div class="middle-info-wrapper main_item_wrapper">
                                ...
                                <? if ($arResult["OFFERS"] && $showCustomOffer): ?>
                                    ...
                                    //webfly start
                                    if ($arItemJSParams["TREE_PROPS"]) {
                                        foreach ($arItemJSParams["TREE_PROPS"] as &$prop) {
                                            if ($prop['CODE'] == 'POMOL') {
                                                $prop['DISPLAY_TYPE'] = 'DROPDOWN-DIV';
                                            }
                                        }
                                    }
                                    //webfly end
                                        ...
                                        <div class="brand-detail-info__preview">
                                            ...
                                            <? if ($arResult['SECTION']): ?>
                                                <div class="link font_xs"><a
                                                            href="<?= $arResult['SECTION']['SECTION_PAGE_URL'] ?>filter/brand-is-<?= $arResult['BRAND_ITEM']['CODE'] ?>/apply/"
