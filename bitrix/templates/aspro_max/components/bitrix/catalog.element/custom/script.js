...
(function (window) {
    ...
    window.JCCatalogElement = function (arParams) {
        ...
        this.skuVisualParams = {
            ...
            'DROPDOWN-DIV'://webfly новый блок для помолов
                {
                    TAG_BIND: 'dropdown-div',
                    TAG: 'dropdown-div',
                    ACTIVE_CLASS: 'active',
                    HIDE_CLASS: 'missing',
                    EVENT: 'click',
            ...
            for (i = index + 1; i < this.treeProps.length; i++) {
                ...
                if (this.config.showAbsent) {
                    ...
                    for (j = 0; j < arShowValues.length; j++) {
                        ...
                        //webfly start
                        if (parseInt(i+1) == this.treeProps.length){
                            prices[arShowValues[j]] = this.GetPrice(tmpFilter, i);
                        }
                        //webfly end
                    }
                }
                ...
                this.UpdateRow(i, arFilter[strName], arShowValues, arCanBuyValues, prices);//webfly add prices param
            }
    ...
    window.JCCatalogElement.prototype.UpdateRow = function (intNumber, activeID, showID, canBuyID, prices) {//webfly add prices param
        var i = 0,
            showI = 0,
            value = '',
            countShow = 0,
        ...
        if (-1 < intNumber && intNumber < this.obTreeRows.length) {
            ...
            if (!!RowItems && 0 < RowItems.length) {
                ...
                for (i = 0; i < RowItems.length; i++) {
                    ...
                    if (prices){
                        if(prices.hasOwnProperty(value)){
                            priceBlock = RowItems[i].querySelector('.sku-price_additional');
                            if (priceBlock){
                                priceBlock.innerText = prices[value];
                            }
                        }
    ...
    //webfly
    window.JCCatalogElement.prototype.GetPrice = function(arFilter, intNumber){
        var propMode = this.treeProps[intNumber].DISPLAY_TYPE,
            RowItems = BX.findChildren(this.obTreeRows[intNumber].LIST, {tagName: this.skuVisualParams[propMode].TAG}, false),
            i = 0,
            j = 0, k = 0,
            check = false, price = '';
                        ...
                        for (propId in arFilter){
                            if(arFilter.hasOwnProperty(propId)) {
                                if (propId == 'PROP_'+arTreeItem[0] && arTreeItem[1] == arFilter[propId]){
                                    price = this.offers[i].PRICE.PRINT_DISCOUNT_VALUE;
                                }
                            }
                        }
                    }
