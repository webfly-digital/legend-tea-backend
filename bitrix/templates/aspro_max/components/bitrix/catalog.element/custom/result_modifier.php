...
if ($productSlider) {
    foreach ($productSlider as $i => $arImage) {
        ...
        if (\Bitrix\Main\Loader::includeModule("delight.webpconverter")) {
            $arImage["SRC"] = DelightWebpConverter::ConvertImage($arImage["SRC"], $arImage["SRC"] . ".webp");
            $ar["BIG"]['src'] = DelightWebpConverter::ConvertImage($ar["BIG"]['src'], $ar["BIG"]['src'] . ".webp");
            $ar["SMALL"]['src'] = DelightWebpConverter::ConvertImage($ar["SMALL"]['src'], $ar["SMALL"]['src'] . ".webp");
            $ar["THUMB"]['src'] = DelightWebpConverter::ConvertImage($ar["THUMB"]['src'], $ar["THUMB"]['src'] . ".webp");
        }
        $productSlider[$i] = array_merge($arImage, $ar);
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])) {
    ...
    $arFilterProp = array();
    $arNeedValues = array();
    $arResult = \Webfly\Helper\Functions::catalogOffersRoz($arResult);
            ...
            if (!empty($arOffer['DISPLAY_PROPERTIES'])) {
                ...
                foreach ($arOffer['DISPLAY_PROPERTIES'] as &$arOneProp) {
                    ...
                    if (is_array($arOneProp['DISPLAY_VALUE'])) {
                        if (count($arOneProp['DISPLAY_VALUE']) > 1) {
                            $dValue = implode(', ', $arOneProp['DISPLAY_VALUE']);
                        } else {
                            $dValue = $arOneProp['DISPLAY_VALUE'];
                        }
                    } else {
                        $dValue = $arOneProp['DISPLAY_VALUE'];
                    }
...
$db_res = CIBlockElement::GetList(array("SORT" => "ASC"), array("ID" => $arResult['ID']), false, false, ['PROPERTY_OPISANIE_DLYA_SAYTA']);
while ($res = $db_res->GetNext()) {
    $arResult['DETAIL_TEXT'] = $res['~PROPERTY_OPISANIE_DLYA_SAYTA_VALUE'];
}
