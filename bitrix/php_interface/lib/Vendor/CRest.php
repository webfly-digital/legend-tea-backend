...
class CRest {
    ...
    private static $last_query_ts = 0;
    ...
    protected static function callCurl($arParams) {
        ...
        if ($arSettings !== false) {
            ...
            try {
                ...
                curl_setopt(
                    $obCurl, CURLOPT_FOLLOWLOCATION, (isset($arParams['followlocation'])) ? $arParams['followlocation'] : 1
                );
                ...
                static::waitMinimumMsInterval();
                $out = curl_exec($obCurl);
                static::setLastQueryTimestamp();
    ...
    public static function setLog($arData, $type = '') {
        ...
        if (!defined("C_REST_BLOCK_LOG") || C_REST_BLOCK_LOG !== true) {
            ...
            $path .= date("Y-m-d H:i:s") . '_' . $type . '_' . rand(1, 9999999) . 'log';
    ...
    /**
     * Запоминает время выполнения
     * последнего запроса к Б24
     */
    private function setLastQueryTimestamp()
    {
        static::$last_query_ts = array_sum(explode(' ', microtime()));
    }

    /**
     * Устанавливает задержку в 0,6 секунды
     * между выполнением запросов к Б24
     */
    private function waitMinimumMsInterval()
    {
        $currentMicroTime = array_sum(explode(' ', microtime()));
        $diffMs = round($currentMicroTime - static::$last_query_ts, 3) * 1000;
        $msToWait = 600;
        if ($diffMs < $msToWait) {
    ...
    /**
     * Рекурсивный вызов списочных методов,
     * возвращает полный список элементов
     * @param $methodName
     * @param $methodParams
     * @param array $listMerge
     * @return array
     */
    public function recursiveGetList($methodName, $methodParams, $listMerge = array())
    {
        $list = self::call($methodName, $methodParams);
        if (!empty($list['error'])) {
            self::setLog(
                array(
        ...
        if (!empty($list['result'])) {
            $result = array_merge($listMerge, $list['result']);
            if (count($list['result']) < $list['total'] && $list['next']) {
                $methodParams['start'] = $list['next'];
                $result = self::recursiveGetList($methodName, $methodParams, $result);
                return $result;
            } else {
                return $result;
            }
