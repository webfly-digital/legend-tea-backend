...
<? if ($arPost["PARAMS"]): ?>
    ...
    if ($obCache->InitCache($cacheTime, $cacheID, $cachePath)) {
        ...
        $rsElements = CIBLockElement::GetList(array($arPost["PARAMS"]["OFFERS_SORT_FIELD"] => $arPost["PARAMS"]["OFFERS_SORT_ORDER"], $arPost["PARAMS"]["OFFERS_SORT_FIELD2"] => $arPost["PARAMS"]["OFFERS_SORT_ORDER2"]), $arFilter, false, array("nTopCount" => 50), $arSelect);
    ...
    <script>
        ...
        GetOfferPrice = function (arFilter, intNumber) {
            var RowItems = BX.findChildren($('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu .item_wrapper:eq(' + intNumber + ') .sku_items_wrapper')[0], {className: 'sku_item'}, false);
            var j = 0, k = 0, price = '';

            if (!!RowItems && 0 < RowItems.length) {

                for (var i in obOffers) {
                    var check = false;
                    for (j in arFilter) {
                        if (arFilter[j] == obOffers[i].TREE[j]) {
                            check = true;
                    ...
                    if (check) {
                        var propId, treeValue, arTreeItem;
                        for (k = 0; k < RowItems.length; k++) {
                            treeValue = RowItems[k].dataset.treevalue;
                            arTreeItem = treeValue.split('_');
                            for (propId in arFilter) {
                                if (arFilter.hasOwnProperty(propId)) {
                                    if (propId == 'PROP_' + arTreeItem[0] && arTreeItem[1] == arFilter[propId]) {
                                        price = obOffers[i].MIN_PRICE.PRINT_DISCOUNT_VALUE;
                                    }
                                }
                            }
                        }
            ...
            if (typeof offerShowPreviewPictureProps === 'object' && offerShowPreviewPictureProps.length) {
                ...
                for (var i = 0, cnt = $obTreeRows.length; i < cnt; ++i) {
                    if (BX.util.in_array($obTreeRows.eq(i).find('>div').data('id'), offerShowPreviewPictureProps)) {
                        var RowItems = BX.findChildren($obTreeRows.eq(i).find('.sku_items_wrapper')[0], {className: 'sku_item'}, false);
                        if (!!RowItems && 0 < RowItems.length) {
        ...
        UpdateRow = function (intNumber, activeID, showID, canBuyID) {
            ...
            if (-1 < intNumber && intNumber < $('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu .item_wrapper').length) {
                ...
                var itemSelector = (selectMode ? {tagName: tag} : {className: 'sku_item'}),
                    hideClass = (selectMode ? 'hidden' : 'missing');

                RowItems = BX.findChildren($('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu .item_wrapper:eq(' + intNumber + ') .sku_items_wrapper')[0], itemSelector, false);
                ...
                if (!!RowItems && 0 < RowItems.length) {
                    ...
                    var listWrapper = $('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu .item_wrapper:eq(' + intNumber + ') .sku_items_wrapper');
                    ...
                    for (i = 0; i < RowItems.length; i++) {
                        ...
                        if (selectMode) {
                            ...
                            obData.props.className += ' sku_item';
                        }
                    ...
                    if (selectMode) {
                        if ($('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu .item_wrapper:eq(' + intNumber + ') .sku_items_wrapper').parent().hasClass('ik_select'))
                            $('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu .item_wrapper:eq(' + intNumber + ') .sku_items_wrapper').ikSelect('reset');
                    }
                }
            }
        };
        ...
        console.log(obOffers);
        if (arShowValues && BX.util.in_array(strPropValue, arShowValues)) {
            if ($('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu').data('selected'))
                selectedValues = $('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu').data('selected');

            var allValues = $('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu .item_wrapper');

            arFilter[strName] = strPropValue;
            for (i = ++depth; i < allValues.length; i++) {
                strName = 'PROP_' + $('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?> .bx_catalog_item_scu .item_wrapper:eq(' + i + ') > div').data('id');
                ...
                for (j = 0; j < arShowValues.length; j++) {
                    ...
                    /*if (parseInt(i + 1) == allValues.length) {
                        prices[arShowValues[j]] = GetOfferPrice(tmpFilter, i);
                    }*/
                }
                <?else:?>
